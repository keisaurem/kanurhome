...
{
    public class OpenDataShelterModels : INearPlaces
    {
        [CsvColumn("ID")]
        ...
        public static OpenDataShelterModels[] GetModels(Context context)
        {

            var csvNames = new string[] {
                "csv/shelters/09gakkou.csv",
            ...
            var shelterSources = csvNames.AsParallel().SelectMany(m =>
            {
                using (var am = context.Resources.Assets.Open(m))
                {
                    ...
                    var ret = new List<OpenDataShelterModels>();
                    for (var i = 0; i < dict.First().Value.Count; ++i)
                    {
                        var model = new OpenDataShelterModels();
